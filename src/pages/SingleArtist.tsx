import CustomSpinner from '@/components/CustomSpinner';
import { SingleArtist, TableData } from '@/types/types';
import { supabase } from '@/utils/supabase';
import { useEffect, useMemo, useState, useContext } from 'react';
import { useParams, useSearchParams } from 'react-router';
import { columns } from "@/components/columns";
import { DataTable } from "@/components/data-table";
import { AppContext } from "@/context/appContext";
import { addSongToPlaylist } from "@/utils/crud";


const SingleArtist = () => {
  const { artistName } = useParams() as { artistName: string };
  const { currentPlaylistId, currentPlaylistName, currentPlaylistCount, setCurrentPlaylistCount } = useContext(AppContext);

  const [tableData, setTableData] = useState<TableData[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [selectedArtists, setSelectedArtists] = useState<string[]>([artistName])
  const [artist, setArtist] = useState<SingleArtist | null>(null);

  useEffect(() => {
    const cachedData = localStorage.getItem("tableData");
    if (cachedData) {
      setTableData(JSON.parse(cachedData));
      setIsLoading(false);
      return;
    }
    supabase
      .from('songs')
      .select(
        `
        song_title:title,
        song_id,
        year,
        ...artists!inner(
          artist_name,
          artist_id
        ),
        ...genres!inner(
          genre_name,
          genre_id  
        )
        `,
      )
      .order('title')
      .then(({ data }) => {
        setTableData(data ?? []);
        localStorage.setItem("tableData", JSON.stringify(data));
        setIsLoading(false);
      });
  }, []);

  useEffect(() => {
    supabase
      .from('artists')
      .select(
        `
          artist_name,
          artist_image_url,
          spotify_desc,
          spotify_url,
          ...types!inner(
            type_name
          )
          `,
      )
      .eq('artist_name', artistName)
      .single()
      .then(({ data }) => {
        setArtist(data);
        setIsLoading(false);
      });
  }, []);

  const [searchParams] = useSearchParams();
  useEffect(() => {
    const artistId = searchParams.get("artist");
    if (artistId) {
      const match = tableData.find(row => row.artist_id === Number(artistId));
      if (match) setSelectedArtists([match.artist_name]);
    }

    // const genreId = searchParams.get("genre");
    // if (genreId) {
    //   const match = tableData.find(row => row.genre_id === Number(genreId));
    //   if (match) setSelectedGenres([match.genre_name]);
    // }
  }, [tableData])

  const handleAddToPlaylist = async (songId: number) => {
    if (!currentPlaylistId) return
    await addSongToPlaylist(songId, currentPlaylistId, currentPlaylistName, currentPlaylistCount, setCurrentPlaylistCount)
  }

  const filteredData = useMemo(() => {
    return tableData.filter((row) => {
      if (selectedArtists.length && !selectedArtists.includes(row.artist_name)) {
        return false;
      }
      return true;
    })
  }, [tableData, selectedArtists])

  return (
    <>
      {isLoading || !artist ? 
        <>
          <CustomSpinner />
        </> :
        <>
          <div className="flex flex-col gap-6 p-8">
            <h1 className="text-3xl font-bold text-center">{artist.artist_name}</h1>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="max-w-xl mx-auto">
                <img className="w-60 h-60 rounded-lg object-cover mx-auto" src={artist.artist_image_url} alt={artist.artist_name} />
              </div>
              <div className="flex flex-col gap-2 pl-10">
                <p className="text-muted-foreground">Artist Type: {artist.type_name}</p>
                <p className="text-muted-foreground">Description: {artist.spotify_desc}</p>
                <a className="text-muted-foreground" href={artist.spotify_url}>Spotify URL: {artist.spotify_url}</a>
              </div>
            </div>

            <div className="w-full px-4 pt-6 sm:w-11/12 md:w-[87.5%] mx-auto overflow-x-auto pb-8">
              <DataTable columns={columns(handleAddToPlaylist)} data={filteredData} />
            </div>

          </div>
        </>
      }
    </>
  );
};

export default SingleArtist;